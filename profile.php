...
<?php
if(($_SESSION['admini'])==0){  echo '<p class="texto444"> Utilizador </p>'; }
...
<?php
echo '<form action="modificarUtilizador.php"  method="POST" name="formModifica">
...
?>
		...
		<div class="descr2"><br><?php echo 'Contacte: '. $nomeuser.'<br> pelo seu e-mail:<br>'. $email ?>
		<!-- The text field -->
		<!-- The button used to copy the text -->
		<button onclick="myFunction()">Copiar E-Mail</button> </div>
...
<?php
	$i=0;
	$sum=0;
		//MEDIA DOS RATINGS E TOTAL DE RATINGS
...
?>
<?php
//---------------------busca dados onde o utilizador atual é o id_user2 e --------------
...
<?php
		...
		$query6 = "SELECT * FROM list WHERE who=$iduser";
  		//prepare the statement				
		$statement6 = mysqli_prepare($db, $query6);
		$result6 = mysqli_stmt_execute($statement6);
		$result6 = mysqli_stmt_get_result($statement6);  
?>


	<div style="margin-top:-50px" class="titulop1">Que Pokemons Precisa</div>

<?php
		'<div style="text-decoration: none">';
		echo '<table class="tabela">
		<tr>
		<th>Que Pokemon</th></tr>' ;
		while( $row = mysqli_fetch_assoc($result6) )
		{
			
			$query7 = "SELECT * FROM pokemon WHERE id_pokemon=$row[what]";		
			$statement7 = mysqli_prepare($db, $query7);
			$result7 = mysqli_stmt_execute($statement7);
			$result7 = mysqli_stmt_get_result($statement7);
